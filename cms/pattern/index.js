...
function getHTMLFiles(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory() && file !== 'node_modules' && file !== 'pms') {
...
async function findAndReplaceInHTML(filePaths, findHTML, replaceHTML) {
  ...
  const userResponse = await questionAsync(`Do you want to replace the content in all matching files? (yes/no): `);
  if (userResponse.toLowerCase() === 'yes') {
    for (const filePath of matchingFiles) {
      ...
      const foundElements = $(findHTML);
      foundElements.replaceWith(replaceHTML);
...
findAndReplaceInHTML(htmlFiles, findHTML, replaceHTML);
